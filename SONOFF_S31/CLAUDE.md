...
```
scp D:/WORK/PLUG/build/espota/firmware.bin bsd:/POOL/Packages/TEMP/FIRMWARE/plug.bin
```
...
- `loop()` is three `avp::Periodically<Fn>::Run(ms)` jobs — button check 100 ms, boot-log 1 s (a no-op after the first connected pass), fleet pull-OTA 60 s — plus two things that run on **every** pass: `ReadCse7766()` and `avp::StaticWebServer::call_in_loop()`, then `yield()`. No FreeRTOS tasks. The meter read is deliberately not periodic: the CSE7766 emits a frame every ~50 ms into a 256-byte UART buffer, and a dropped frame costs pulse timing and update flags, so anything that blocks `loop()` for ~533 ms starts losing frames.
- `StaticWiFi_Conn` runs the status LED from a hardware timer (`StaticWiFi_Conn::LED_pin = LED` plus `Opts.status_indication_func_ = avp::StaticWiFi_Conn::Blinken`), so the indication keeps working even when `loop()` is blocked. `AVP_RAM_ATTR` is `IRAM_ATTR` (see `platformio.ini`) — anything called from the timer ISR must live in IRAM.
...
`/read` returns `Voltage[V] Current[A] Power[W] Energy[Wh] RelayStatus`. Energy accumulates from boot until `/energy_reset`; nothing clears it automatically, and it is printed to 4 decimals because one meter pulse is only ~0.0016 Wh.
